...
export function useUnifiedVoiceHandler({
    ...
    try {
      ...
      const monitorAudioLevel = () => {
        if (analyserRef.current) {
          ...
          // Simple interruption detection during AI speech
          if (voiceState === 'ai_speaking' && average > 60) {
  ...
  const startListening = useCallback(async (): Promise<boolean> => {
    console.log('🎤 Starting unified voice recognition');
    
    if (!isSupported || disabled || isInitializingRef.current) {
      return false;
    }

    // Don't start if AI is speaking (unless this is an interruption)
    ...
    try {
      ...
      recognition.onresult = (event: any) => {
        // Block if AI is speaking (except during interruption recovery)
        if (voiceState === 'ai_speaking' && voiceState !== 'interrupted') {
